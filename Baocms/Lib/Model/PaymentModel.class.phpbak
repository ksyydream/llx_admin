...
class PaymentModel extends CommonModel {
   protected $pk = 'payment_id';
    ...
    public function getPayments($mobile = false) {
        $datas = $this->where('is_open',1)->select();
        $return = array();
...
//					fclose($open);
					foreach($goods as $v){
						if($v['is_yhk'] == 1){//优惠卡规则
							...
							$zengpins = $Zengpin->where(array('goods_id'=>$v['goods_id']))->select();
							if($user['zp']){//赠品
...
//									fclose($open);
									...
									$zp = $zp_old2;
//
//									if($kk == $v['shop_id']){//相同店铺
//										$vv_array = (array)$vv;
//										foreach($zengpins as $vvv){
//											if(isset($vv_array[$vvv['desc']])){
//												$vv_array[$vvv['desc']] = $vv_array[$vvv['desc']] + $vvv['qty'];
//											}else{
//												$vv_array[$vvv['desc']] = $vvv['qty'];
//											}
//										}
//										$zp[$v['shop_id']] = (object)$vv_array;
//									}else{//不同店铺
//										$vv_array = array();
//										foreach($zengpins as $vvv){
//											$vv_array[$vvv['desc']] = $vvv['qty'];
//										}
//										$zp_old[$kk] = (object)$vv_array;
//										$zp = $zp_old;
//
//										$open=fopen('/var/www/html/baocms/Baocms/Lib/Payment/logs/'.date( 'Y-m-d' ) . '.zp.log',"a" );
//										fwrite($open,var_export($zp,true));
//										fclose($open);
//									}
							...
							$Users->where(array('user_id'=>$logs['user_id']))->save(array('zp'=>json_encode($zp)));
							//三级分销
							...
							if(isset($parent[$v['shop_id']])){
								...
								$fx=D('shop')->where(array('shop_id'=>$parent[$v['shop_id']]))->find();
//								$users->addIntegral($uid1, $v['mall_price']*0.2, '第一层分红获得秀币');
								D('Users')->Money($uid1, $v['mall_price']*$fx['fx_1']/100, '第一层提成获得');
...
//									$users->addIntegral($uid2, $v['mall_price']*0.1, '第二层分红获得秀币');
									D('Users')->Money($uid2, $v['mall_price']*$fx['fx_2']/100, '第二层提成获得');
...
//										$users->addIntegral($uid2, $v['mall_price']*0.05, '第三层分红获得秀币');
										D('Users')->Money($uid3, $v['mall_price']*$fx['fx_3']/100, '第三层提成获得');
									}
								}
							}
						}
...
//                    ));
					...
					D('Users')->Money($shop['user_id'],$logs['need_pay'],'优惠买单，订单号：' . $logs['order_id']);




//					D('Users')->addMoney($shop['user_id'], $logs['need_pay'], '用户买单，订单号:' . $pay['order_id']);//写入商户资金
					
//                    $youhui = D('Shopyouhui')->where(array('shop_id'=>$pay['shop_id']))->find();
                    D('Pay')->save(array('id' => $logs['order_id'], 'status' => 2)); //设置已付款
...
//					D('Users')->where(array('user_id'=>$shop['user_id']))->setInc('integral',$pay['integral']);
                    ...
                    $_data_balance = array(
                        ...
                        'topcolor' => '#F55555',
                        'first' => '亲,您的账户有变动',
                        'remark' => '有疑问请联系拉拉秀：' . $this->CONFIG['site']['tel'],
                        'accountType' => '会员账户',
                        'operateType' => '费用支出',
                        'operateInfo' => '优惠买单',
                        'limit' => '-' . $limit . '元',
                    ...
                    if (empty($logs['order_id']) && !empty($logs['order_ids'])) {//合并付款
						...
						D('Order')->overOrder($logs['order_id']); //确认到账入口
