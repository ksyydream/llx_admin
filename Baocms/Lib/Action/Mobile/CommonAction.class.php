...
//            var_dump($_COOKIE['openid']);die;
            ...
            /*if (!$_COOKIE['openid']) {
                if ($code = $_REQUEST['code']) {
                    $client = D('Weixin')->admin_wechat_client('',$appid,$appsecret);
                ...
                if(!D('Connect')->where(array('open_id'=>$_COOKIE['openid']))->find()){
                    D('Connect')->add(array(
...
//            $user = D('Users')->getUserByOpenid($_COOKIE['openid']);
//            setUid($user['user_id']);
//            $this->user = $user;
...
//            var_dump($_COOKIE['openid']);die;
		...
		//对微信用户是否登录进行判断
        if ($this->_CONFIG['sit e']['weixin'] == 1) {
            die;
            if ($is_weixin && !empty($this->_CONFIG['weixin']['appid'])) {
    //            die($this->_CONFIG['weixin']['appid']);
...
//        die($this->parseTemplate($templateFile));
        $this->seo();
    ...
    protected function fengmiMsg($message, $jumpUrl = '', $time = 1000){
    ...
    protected function fengmiErrorJump($message, $jumpUrl = '', $time = 1000){
