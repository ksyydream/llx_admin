		...
		if ($tuan['tstatus'] == 0) {
			...
			if ($tuan['renshu'] <= $tuanrenCount) {
				...
				foreach ($Ptuanteam as $key => $val) {
					...
					'url' => "http://" . $_SERVER['HTTP_HOST'] . "/mobile/pintuan/tuan/id/" . $tuan['tuan_id'] . ".html", 'topcolor' => '#F55555', 'first' => '您参团的商品［' . $tuan['goods_name'] . '］已组团成功，即将发货，请注意查收！', 'payprice' => round($detail['need_pay'] / 100, 2) . '元', 'orderno' => $detail['log_id'], 'remark' => '点击查看订单详情', );
						...
						if ($tstatus == 0) {
							...
							if ($Porder['renshu'] <= $tuanrenCount) {
								...
								foreach ($Ptuanteam as $key => $val) {
									...
									include_once "Baocms/Lib/Net/Wxmesg.class.php";
									$_data_ctsuccess = array(//整体变更
									'url' => "http://" . $_SERVER['HTTP_HOST'] . "/mobile/pintuan/tuan/id/" . $tuan_id . ".html", 'topcolor' => '#F55555', 'first' => '您参团的商品［' . $Porder['goods_name'] . '］已组团成功，即将发货，请注意查收！', 'payprice' => round($order['need_pay'] / 100, 2) . '元', 'orderno' => $order['log_id'], 'remark' => '点击查看订单详情', );
									$ctsuccess_data = Wxmesg::ctsuccess($_data_ctsuccess);
									$return = Wxmesg::net($uid, 'OPENTM401153728', $ctsuccess_data);
									//结束
									//====================拼团成功通知===============================
		...
		switch ($logs['type']) {
			...
			case 'goods' :
				if (empty($logs['order_id'])) {
					...
					$this -> goods_success('恭喜您支付成功啦1！', $logs);
				}
				break;
			case 'ding' :
			...
			case 'pintuan' :
				...
				break;
			default :
				$this -> success('恭喜您支付成功啦', U('mcenter/index/index'));
				break;
		}
	}
