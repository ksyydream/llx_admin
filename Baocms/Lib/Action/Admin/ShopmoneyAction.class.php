...
class ShopmoneyAction extends CommonAction{
    ...
    public  function yhkmoney(){
        $Yhkmoney = D('Ordergoods');
        import('ORG.Util.Page');// 导入分页类
        $map = array();
        if(($bg_date = $this->_param('bg_date',  'htmlspecialchars') )&& ($end_date=$this->_param('end_date','htmlspecialchars'))){
            $bg_time = strtotime($bg_date);
        ...
        if ($map){
            $olist = $Yhkmoney->where($map)->order(array('id'=>'desc'))->select();
        }else{
            $count      = $Yhkmoney->where($map)->count();// 查询满足要求的总记录数
            $Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
            $show       = $Page->show();// 分页显示输出
        $olist = $Yhkmoney->where($map)->order(array('id'=>'desc'))->limit($Page->firstRow.','.$Page->listRows)->select();}
        foreach ($olist as $v){
        ...
        $shop_detail=D('Shop')->itemsByIds($shop_ids);
        foreach($list as  $k=>$z){
            $fx=(100-$shop_detail[$z['shop_id']]['fx_1']-$shop_detail[$z['shop_id']]['fx_2']-$shop_detail[$z['shop_id']]['fx_3'])/100;
            $list[$k]['fx_price'] = $z['total_price']*$fx/100;
            $list[$k]['fx']=$fx;
            $sumfx=$sumfx+$list[$k]['fx_price'];
        }
        //print_r($list);
        $this->assign('shops',$shop_detail);
        $this->assign('sumfx',$sumfx);
        $this->assign('list',$list);// 赋值数据集
        $this->assign('page',$show);// 赋值分页输出
        $this->display(); // 输出模板
    }
