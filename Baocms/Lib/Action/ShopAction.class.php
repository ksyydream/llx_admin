...
{
    private $create_fields = array('user_id', 'cate_id','province_code', 'city_code', 'area_code', 'shop_name', 'logo', 'mobile', 'photo', 'addr', 'tel', 'extension', 'contact', 'tags', 'near', 'is_pei', 'business_time', 'delivery_time', 'orderby', 'lng', 'lat', 'price', 'recognition');
    private $edit_fields = array('user_id', 'cate_id', 'province_code', 'city_code', 'area_code', 'shop_name', 'mobile', 'logo', 'photo', 'addr', 'tel', 'extension', 'contact', 'tags', 'near', 'business_time', 'delivery_time', 'is_pei', 'orderby', 'lng', 'lat', 'price', 'is_ding', 'recognition', 'apiKey', 'mKey', 'partner', 'machine_code','service','service_audit');
...
//        die($detail['user_id']);
    ...
    public function delete2($shop_id = 0)
    {
        if (is_numeric($shop_id) && ($shop_id = (int) $shop_id)) {
            ...
            $obj->delete($shop_id);
            $this->baoSuccess('彻底删除成功！'.$shop_id, U('shop/recovery'));
        } else {
            $shop_id = $this->_post('shop_id', false);
            if (is_array($shop_id)) {
                $obj = D('Shop');
