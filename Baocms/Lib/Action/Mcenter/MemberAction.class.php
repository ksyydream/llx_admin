...
class MemberAction extends CommonAction
{
    ...
    public function pay()
    {
        ...
        if ($detail['type'] == 'ele') {
            ...
            $this->success('恭喜您买单成功', U('mcenter/member/index'));
            die;
        } else {
    ...
    protected function goods_success($message, $detail)
    {
        ...
        D('Order')->overOrder($detail['order_id']);
        $this->display('goods');
    }
    ...
    {
        header('Location: ' .'http://llx.51loveshow.com/personal');
        die;
        ...
        //用户信息
        $user_info = D('Users')->find($this->uid);
        $yhk = (array)json_decode($user_info['yhk']);
        $yhk_s = 0;
        foreach($yhk as $v){
            $yhk_s+=$v;
        }
        $this->assign('money', $user_info['gold']/100);
        $this->assign('yhk', $yhk_s);
