<?php
class ShopmsgAction extends CommonAction {

    private $create_fields = array('city_id','shop_id', 'type', 'title', 'intro', 'link_url', 'create_time', 'details');
    private $edit_fields = array('city_id','shop_id', 'type', 'title', 'intro', 'link_url', 'details');
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Shopmsg');
            if ($obj->add($data)) {
                $this->baoSuccess('添加成功', U('shopmsg/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->assign('types',  D('Msg')->getType());
        ...
        if ($msg_id = (int) $msg_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->baoSuccess('操作成功', U('shopmsg/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($msg_id = 0) {
        if (is_numeric($msg_id) && ($msg_id = (int) $msg_id)) {
            $obj = D('Shopmsg');
            $obj->delete($msg_id);
            $this->baoSuccess('删除成功！', U('shopmsg/index'));
        } else {
